...
Connection.prototype.createDB = function createDB(name,callback){
    var options = {};
    options.sql = 'CREATE DATABASE '+name+';';
    options.values = null;
    return this.query(options, callback);
};


Connection.prototype.dropDB = function dropDB(name,callback){
    var options = {};
    options.sql = 'DROP DATABASE '+name+';';
    options.values = null;
    return this.query(options, callback);
};

Connection.prototype.ifDB = function ifDB(name){
   var self = this;
     var object = {
       exist :function (cb) {
         var options = {};
         options.sql = 'SHOW DATABASES LIKE \''+name+'\';';
         options.values = null;
         self.query(options, function(err,rows){
           cb(rows.length)
         });
        },
        inUse :function(cb) {
            var options = {};
            options.sql = 'select DATABASE() ;';
            options.values = null;
            self.query(options, function(err,rows){
                cb(rows);
            });
...
Connection.prototype.Use = function Use(name,callback){
    var options = {};
    options.sql = 'USE ' + name + ' ;';
    options.values = null;
    return this.query(options, callback);
};

Connection.prototype.Put = function Put(name,columns,callback){

  var request = "INSERT INTO " + name + " ";
  var values = "(";
  var prepared = "(";

  var keyNames = Object.keys(columns);
  for (var i in keyNames) {
      switch(typeof(eval("columns."+keyNames[i]))){
          case "string":
              values += "'"+eval("columns."+keyNames[i])+"',";
              break;
          case "number":
              values += eval("columns."+keyNames[i])+",";
              break;
    ...
    values += ")";
    prepared =  prepared.substr(0, prepared.length-1);
    prepared += ")";
    request += prepared + " VALUES " + values;
...
//INSERT INTO jeux_video (ID) VALUES ('Battlefield 1942', 'Patrick')
